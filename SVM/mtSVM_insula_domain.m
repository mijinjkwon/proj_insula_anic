function mtSVMs_wOptNcv_1vO_insula_linear_domain_m2(task_id)
    ...
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2 = struct('model_n', [], 'model', [], ...
    ...
    classNames = {'Pain','Appetitive Process','Aversive Process', 'Cognitive Control'};
    tabulate(Y)
    ...
    Mdl = fitcecoc(X,Y, 'Coding','onevsall', 'Learners',t, 'ClassNames',classNames, ...
    ...
    [label_pred, NegLoss_pred, PBScore_pred] = predict(Mdl, dat_test.dat');


    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).model_n = model_current;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).model = Mdl;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).resub_results.label_resub = label_resub;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).resub_results.NegLoss_resub = NegLoss_resub;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).resub_results.PBScore_resub = PBScore_resub;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).resub_results.label_true_train = dat_train.metadata_table.Domain;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).pred_results.label_pred = label_pred;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).pred_results.NegLoss_pred = NegLoss_pred;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).pred_results.PBScore_pred = PBScore_pred;
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2(n).pred_results.label_true_test = dat_test.metadata_table.Domain;

    % Results without model
    results_mtSVMs_wOptNcv_1vO_insula_linear_m2_woMdl = rmfield(results_mtSVMs_wOptNcv_1vO_insula_linear_m2, 'model')

    % Save the results
    output_file = sprintf('Results_wOptNcv_1vO_insula_linear_domain_m2_%d.mat', n); % Create filename with array task id
    output_file_woMdl = sprintf('woMdl_Results_wOptNcv_1vO_insula_linear_domain_m2_%d.mat', n); % Create filename with array task id

    current_output_dir = fullfile(output_dir, 'mtSVM_domain_linear_re_2')
    save(fullfile(current_output_dir, output_file), 'results_mtSVMs_wOptNcv_1vO_insula_linear_m2', '-v7.3') % whole
    save(fullfile(current_output_dir, output_file_woMdl), 'results_mtSVMs_wOptNcv_1vO_insula_linear_m2_woMdl', '-v7.3') % without model
end